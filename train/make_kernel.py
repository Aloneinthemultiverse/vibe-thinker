"""Generate a self-contained Kaggle GPU kernel that fine-tunes the tool-calling
LoRA and converts the result to GGUF. The SFT data is embedded inline (base64) so
the kernel needs no attached dataset — just GPU + internet.

Run:  python -m train.make_kernel
Then: kaggle kernels push -p kaggle_kernel
"""
import base64
import json
import os
...
OUT = os.path.join(ROOT, "kaggle_kernel")
USER = "sujitnarrayan"
SLUG = "vibethinker-tooluse-finetune"

with open(os.path.join(ROOT, "data", "sft_all.jsonl"), "rb") as f:
...
META = {
    ...
    "dataset_sources": ["sujitnarrayan/vibethinker-sft-15k"],
    "competition_sources": [],
    "kernel_sources": [],
    "model_sources": [],
}
...
print(f"wrote {OUT}/kernel.py ({len(KERNEL)} bytes, data {len(DATA_B64)} b64 chars)")
print(f"wrote {OUT}/kernel-metadata.json -> {META['id']}")
